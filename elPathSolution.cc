...
static const float EPS_SIMILAR_PATHS= 1.0e-3;
static const float EPS_DEGENERATE_POLYGON_AREA        = 1.0e-6;
...
void PathSolution::solve(void)
{
  ...
  int numBuckets = 
    (m_solutionNodes.size() + DISTANCE_SKIP_BUCKET_SIZE - 1) 
    / DISTANCE_SKIP_BUCKET_SIZE;
				...
				Vector4& failPlane)
{
	...
	if (missed)
	{
		...
		} else
		{
			...
			for (int i=2; i < beam.numPleqs(); i++)
				if (dot(missRay.m_b, beam.getPleq(i)) < 
				    dot(missRay.m_b, missPlane))
	...
	for(std::multimap<float, int>::const_iterator it = 
	      m_pathFirstSet.upper_bound(fmin);
	    ...
	    ++it)
	{
		...
		{
			if ((p.m_points[j] - path.m_points[j]).lengthSqr() > 
			    EPS_SIMILAR_PATHS * EPS_SIMILAR_PATHS)
			{
				safe = true;
				break;
			}
