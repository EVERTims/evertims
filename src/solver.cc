  ...
  for (i = m_lastMappedSolutionNode + 1 ; i <= m_lastAvailableSolutionNode; i++) 
    {
      ...
      if (m_solutionNodes[i].m_writers.size () > 0)
	m_solutionNodeMap[id] = &(m_solutionNodes[i]);
    }
...
void Solver::update ()
{
  ...
  if (m_newSolutionNodesAvailable)
    mapAvailableSolutionNodes ();
